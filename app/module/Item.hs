...
restockItem oldLogItemList choice amount = do
    ...
    if (extractItem itemExist) == UnknownItem
        then putStrLn "Barang tidak ditemukan! Cek kembali ID Barang!"
        else
            if amount == 0
                then putStrLn "Jumlah yang dimasukkan nol. Anda yakin benar memasukkan jumlah ini?"
                else putStrLn "Sukses mengisi ulang barang!"
...
takeItem oldLogItemList choice amount = do
    ...
    if (extractItem itemExist) == UnknownItem
        then putStrLn "Barang tidak ditemukan! Cek kembali ID Barang!"
            ...
            if amount > (storage $ extractItem itemExist)
                then putStrLn "Jumlah tidak cukup untuk mengambil barang."
                else
                    if amount == 0
                        then putStrLn "Jumlah yang dimasukkan nol. Anda yakin benar memasukkan jumlah ini?"
                        else putStrLn "Sukses mengisi ulang barang!"
